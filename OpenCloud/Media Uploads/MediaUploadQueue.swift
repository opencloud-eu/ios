...
class MediaUploadQueue : OCActivitySource {
	...
	func setup() {
		...
		// Try striking a viable compromise between resource usage and performance by using
		// half the number of cores
		var maxConcurrentOperationCount = ProcessInfo.processInfo.activeProcessorCount / 2
	...
	func scheduleUploads(in bookmark:OCBookmark) {
		...
		OCCoreManager.shared.requestCore(for: bookmark, setup:nil, completionHandler: {(core, _) in
			...
			if let core = core {
				...
				let backgroundTask = OCBackgroundTask(name: "com.opencloud.media-upload-task", expirationHandler: { (bgTask) in
