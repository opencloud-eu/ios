...
class ScheduledTaskManager : NSObject {
	...
	private let taskQueue = DispatchQueue(label: "com.opencloud.task-scheduler.queue", qos: .background, attributes: .concurrent)
	...
	func setup() {
		...
		wifiMonitorQueue = DispatchQueue(label: "com.opencloud.scheduled_task_mgr.wifi_monitor")
		...
		BGTaskScheduler.shared.register(forTaskWithIdentifier: "com.opencloud.background-refresh-task", using: nil) { (task) in
...
extension ScheduledTaskManager {
	...
	func scheduleBackgroundRefreshTask() {
        ...
        let request = BGAppRefreshTaskRequest(identifier: "com.opencloud.background-refresh-task")
