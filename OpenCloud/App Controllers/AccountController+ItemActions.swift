...
extension AccountController: OpenCloudAppShared.DataItemSwipeInteraction {
...
extension AccountController: OpenCloudAppShared.DataItemContextMenuInteraction {
...
extension AccountController: OpenCloudAppShared.DataItemDragInteraction {
	public func provideDragItems(with context: ClientContext?) -> [UIDragItem]? {
		if let bookmark, let userActivity = openInNewWindowUserActivity(with: clientContext) {
			let itemProvider = NSItemProvider(item: bookmark, typeIdentifier: "eu.opencloud.ios.ocbookmark")
...
extension AccountController: OpenCloudAppShared.DataItemDropInteraction {
