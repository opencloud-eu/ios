	...
	@discardableResult func retrieveLinkedItem(with completion: @escaping (_ item: OCItem?, _ bookmark: OCBookmark?, _ error: Error?, _ connected: Bool) -> Void) -> Bool {
		...
		for bookmark in bookmarks {
			...
			if foundItem == nil {
				...
				// E.g. if we would like to use app URL scheme (opencloud://) instead of universal link, to make it work with oC SDK, we need to change scheme back to the original bookmark URL scheme
