//
//  AppStateActionOpenItem.swift
//  OpenCloudAppShared
//
//  Created by Felix Schwarz on 12.06.23.
...
public class AppStateActionOpenItem: AppStateAction {
	var item: OCItem?
	...
	override public func perform(in clientContext: ClientContext, completion: @escaping AppStateAction.Completion) {
		if let item {
			_ = item.openItem(from: nil, with: clientContext, animated: false, pushViewController: true, completion: { success in
...
public extension AppStateAction {
	static func open(item: OCItem, children: [AppStateAction]? = nil) -> AppStateActionOpenItem {
		return AppStateActionOpenItem(item: item, children: children)
	}
}
